    ...
    <div class="gredBox">
      ...
      <div class="whiteTop">
		...
		<?php    $nid		=	$_GET['frmID'];
				...
				$sel_res_id	=	mysql_query("select * from `mnoviforms_response` where `mnoviforms_id`='".$nid."' && `patient_id`='".$pid."' && `Revision`='".$rev_id."'");
				 $hav_res_id	=	mysql_num_rows($sel_res_id);
				if($hav_res_id){
					while($get_res_id	=	mysql_fetch_array($sel_res_id)){
					$res_id	=	$get_res_id['response_id'];
					
				 	$show_q ="select * from `mnoviforms_response_detail` where `response_id`='".$res_id."' && `Value` != 'NA' && `Revision`='".$rev_id."'";

						$sel_res_val	=	mysql_query($show_q);
		  ...
		  <?php if($_GET['frmID'] ==2 || $_GET['frmID'] == 3){ ?>
				...
				<?php if($_GET['frmID'] ==2){echo "Take Health Assessment Top Dysfunctions";} ?><?php if($_GET['frmID'] ==3){echo "Restoration Health Assessment Top Dysfunctions";} ?>
					...
					<span>*Disclaimer: This software is intended to support licensed physicians and other licensed health care providers and aid them in their decision making process.<br>
					 It is not a substitute for professional medical advice, diagnosis or treatment.
					 It has not been evaluated by the FDA. The recommendations of nutrients is provided with the intent of supporting the physiological   and biochemical processes of the human body, and not to diagnose, treat, cure, or prevent any disease or condition.</span><br><br>
