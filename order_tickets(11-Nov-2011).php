...
if($_POST['qty'] && $_POST['id'] || ($_POST['submit'])){}
...
if($_REQUEST['submit']){	
	...
	$priceAfterQty		=	$price*$qty;
		...
		$bc_fee			=	$bc_service_fee+$priceAfterQty;
		$total_price	=	($bc_fee+$total_price)+($finalServiceCharges*$qty);
		}
		else{
...
if($bc_service_fee_type==1 || $bc_service_fee_type==3){
	$grand_total	=	$total_price+$bc_service_fee;
	}
	else{
...
mysql_query("INSERT INTO `orders` (`id`, `user_id`, `total_price`, `date`, `type`, `main_ticket_id`) VALUES (NULL, '$user_id', '$grand_total', '$date', 'ticket', '$main_ticket_id')");
	...
	$persent = getSingleColumn('event_fee',"select * from default_settings");
		if(is_numeric($row['price'])){			
		$admin_persent	=	$row['price']*$persent/(100);
		}
		else{
		$admin_persent = $persent;
		}
		$buyer = getSingleColumn('buyer_event_grabber_fee',"select * from event_ticket where event_id=$bc_event_id");
	...
	$rt = mysql_query("select * from `events` where `id`='$event_id'");
...
?>
                  <table cellpadding="0" cellspacing="0" border="1" bordercolor="#E1E1E1" style="color:#FFFFFF" width="100%" align="center">
                    <tr bgcolor="#383838">
                      <td width="37%" height="30" align="center"><strong>Ticket Title</strong></td>
                      <td width="14%" align="center"><strong>Price</strong></td>
                      <?php
			if ($bc_service_fee_type==2){?>
                      <td width="14%" align="center"><strong>Service Fee</strong></td>
                      <?php } ?>
                      <td width="13%" align="center"><strong>Quantity</strong></td>
                      <td width="13%" align="center"><strong>Date</strong></td>
                      <td width="22%" align="center"><strong>Total</strong></td>
                    </tr>
	...
	for($i=0;$i< count($_POST['id']); $i++) {
		$id		=	$_POST['id'][$i];
		$qty	=	$_POST['qty'][$i];
		echo '<input type="hidden" name="id[]" value="'.$id.'" /><input type="hidden" name="qty[]" value="'.$qty.'" />
...
?>
                    <tr style="color:#000000">
                      <td width="37%" height="30" align="center"><?php echo $row['title']; ?></td>
                      <td width="14%" align="center"><?php if($row['price']!='free' && $row['price']!='donation'){ echo "$".$row['price'];} else{ echo $row['price']; } ?>
                        <br />
                        <small>+ $
                        <?php
			...
			if ($bc_service_fee_type==2){?>
                      ...
                      <td width="13%" align="center"><?php echo $_POST['date'][$i]; ?></td>
