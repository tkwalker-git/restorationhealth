	...
	$have=mysql_query("select * from `schedule_patient` where `patient_id`='$bc_patient_id' && `clinic_id`='$bc_userid'");
		...
		// save data here
		 if ($action1 == "save") {
			 ...
			 $sql	=	"insert into schedule_patient (patient_id,clinic_id,comments,date) values ('" . $bc_patient_id . "','" . $bc_userid . "','" . $bc_comments . "','" . $today . "')";
			$res	=	mysql_query($sql);
...
//////////////////////////////////
	...
	$emr_session_id		= attribValue('users', 'emr_clinic_id', "where id='". $_SESSION['LOGGEDIN_MEMBER_ID'] ."'");
	$emr_doctor_id		= attribValue('users', 'emr_doctor_id', "where id='". $_SESSION['LOGGEDIN_MEMBER_ID'] ."'");
	$emr_patient_id		= attribValue('patients', 'genensysuserid', "where id='".$bc_patient_id."'");
	$request_time		= $one_date."T".$startTime;
	$emr_appt_id 		= addAppointmentRequest($emr_session_id,$emr_patient_id,$request_time,$emr_doctor_id,$bc_comments);

	$sql_date	=	mysql_query("insert into schedule_dates (patient_id,clinic_id,emr_appt_id,cons_date,start_time, end_time) values('" . $bc_patient_id . "','" . $bc_userid . "','" . $emr_appt_id . "','" . $one_date . "','" . $startTime . "','" . $endTime . "')");
	...
	if ($sql_date) {
				...
				/* $sucMessage = $emr_appt_id."|".$emr_doctor_id."|".$emr_patient_id."|".$emr_session_id."|".$request_time."|".$bc_comments; */
		...
		//end save here
	if ($action1 == "edit") {
			 $sql	=	"update schedule_patient set comments = '" . $bc_comments . "' where `patient_id`='$frmID' && `clinic_id`='$bc_userid'";
  ...
  <td class="z-remove-cell z-col-4">
    <a class="z-occurrence-remove"><img src="images/icon_remove.gif" alt="remove" title="remove"></a>
                ...
                <div id="head">Comments</div>
                  <div>
                    <textarea name="event_description" id="event_description" class="bc_input" style="width:868px; height:250px"><?php echo $bc_comments; ?></textarea>
                  </div>
