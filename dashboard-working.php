      ...
      <div class="whiteTop">
        <div class="whiteBottom">
		 ...
		 <div style="padding-bottom:50px; overflow:hidden;">
			...
			<?php if($_SESSION['usertype']=='clinic' || $_SESSION['usertype']=='doctor'){	?>
				...
				<?php if($_SESSION['usertype']=='clinic' || $_SESSION['usertype']=='doctor'){	?>
					...
					<table cellpadding="0" cellspacing="0" border="0" width="100%" class="add_links">
                       ...
                       <tr>
	                        <td><strong>Step 1:</strong> <a href="<?php echo ABSOLUTE_PATH; ?>Clinical_Intake_Form.php" style="color:#0066FF; font-size:15px;"<a onclick="return confirm('Notice: This registration could take up to 30 seconds to process. We appreciate your patience.  Click OK to proceed...');">  Fill-Out Clinical Intake Form</a></td>
                        </tr>
			...
			$doctor_id 	= getSingleColumn("clinicid","select * from `patients` where `id`='".$_SESSION['LOGGEDIN_MEMBER_ID']."'");
			$rest				=	mysql_query("select * from `affiliate` where `clinic_id`='".$doctor_id."'");
			$have_rest		=	mysql_num_rows($rest);
			if($have_rest){
				$aid  =	getSingleColumn("affiliate_id","select * from `affiliate` where `clinic_id`='".$doctor_id."'");
				}
				else {
					$aid = "";
					}
			...
			if($aid && $aid != "")
			{
				$sql10 = "select * from `doctor_forms` where  `doctor_id`='". $doctor_id."' || `doctor_id`='".$aid."'  ORDER BY form_name ASC";
				}
				else
					{
						$sql10 = "select * from `doctor_forms` where  `doctor_id`='".$doctor_id."' ORDER BY form_name ASC";
					}

			$res10 = mysql_query($sql10);
			while ($row10 = mysql_fetch_assoc($res10)){
				$link_name = $row10['link_name'];
				$form_name = $row10['form_name'];
			?>

    	                    <a href="<?php echo ABSOLUTE_PATH; ?>survey.php?NoviFormName=<?php echo $link_name;?>" style="color:#0066FF; font-size:14px;" <a onclick="return confirm('Please Wait: This survey could take up to 10 seconds to load...');"><?php echo $form_name;?></a><span> | </span>
    	                    <?php } ?>
    	                    </td>
                        </tr>
...
<?php
		...
		echo "]";
					...
					?> </span>
                        ...
                        <tr>
        	                <td colspan="3"><a href="<?php echo ABSOLUTE_PATH; ?>clinic_manager.php?p=inactive"style="color:#0066FF; font-size:15px;">Inactive Patients </a>&nbsp;
        	                	<span style="color:#FF0000;">
        	                <?php
		        	                $today=date("Y-m-d");
									$sql = "select * from `patients` where `status` = '1' &&  `clinicid`='". $_SESSION['LOGGEDIN_MEMBER_ID'] ."'";
									$results1 =mysql_query($sql);
									$checkrec	=	mysql_num_rows($results1);
									while($row = mysql_fetch_array($results1))
										{
											$pid = $row['id'];
											$survey_count =  "SELECT count(*) as tot FROM `mnoviforms_response` where `patient_id` = '$pid'";
											$res2 = mysql_query($survey_count);
											}
												while($row2 = mysql_fetch_array($res2)){
													$checkrec2	=	mysql_num_rows($res2);
													$inactive_count = $row2['tot'];
													echo "[".$inactive_count."]";
												}
