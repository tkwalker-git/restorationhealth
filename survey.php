...
function getrespondent_ws($post,$rev_id)
{
  ...
  {
	$sql = "INSERT INTO `mnoviforms_response_detail` (`Id`, `response_id`, `Key`, `Value`,`Revision`) VALUES (NULL, '".$post."', '".$data0[$ci]."', '".$data1[$ci]."', '".$rev_id."')";
	mysql_query($sql);
  }
          ...
          <div class="whiteMiddle" style="padding-top:7px;">
			<div style="padding-bottom:50px; overflow:hidden;">
				...
				if(isset($_GET['sid']) && $_GET['sid'] !="")
				{
					...
					if($res)
					{
				$rev_id = getSingleColumn("Revision","SELECT * FROM `mnoviforms_response` WHERE `PangiaId`='".$member_id."' and `mnoviforms_id` = '".$form_id."' order by id desc limit 0,1");
				$rev_id++;
							...
							getrespondent_ws($_GET['sid'],$rev_id);
?>
