...
if($_GET['id']){
	...
	if(validateID($_SESSION['LOGGEDIN_MEMBER_ID'],'doctor_forms',$_GET['id']) =='false'){
		echo "<script>window.location.href='clinic_manager.php?p=forms';</script>";
	}
}

$bc_form_name		= DBin($_POST["product_name"]);
$bc_description			= DBin($_POST["description"]);
$bc_cost					= $_POST["cost"];
$bc_link_name		= $_POST["product_code"];
$bc_doctor_id				= $_SESSION['LOGGEDIN_MEMBER_ID'];
...
if (isset($_POST["submit"]) ) {
	...
	if (!count($errors)){
		 ...
		 if ($action1 == "save") {
			$sql	=	"insert into doctor_forms (clinic_id,doctor_id,link_name,form_name,form_description) values ('" . $bc_clinic_id . "','" . $bc_doctor_id . "','" . $bc_link_name . "','" . $bc_form_name . "','" . $bc_description . "')";
			$res	=	mysql_query($sql);
			$frmID = mysql_insert_id();
			if ($res) {
				$sucMessage = "Record Successfully inserted.";
				echo "<script>window.location.href='clinic_manager.php?p=forms&id=$frmID';</script>";
			} else {
		...
		if ($action1 == "edit") {
			$sql	=	"update doctor_forms set clinic_id = '" . $bc_clinic_id . "', doctor_id = '" . $bc_doctor_id . "', form_name = '" . $bc_form_name . "', link_name = '" . $bc_link_name . "', form_description = '" . $bc_description . "' where id=$frmID";
			$res	=	mysql_query($sql);
			if ($res) {
...
$sql	=	"select * from doctor_forms where id=$frmID";
$res	=	mysql_query($sql);
if ($res) {
	if ($row = mysql_fetch_assoc($res) ) {
		$bc_form_name	=	DBout($row["link_name"]);
		$bc_description		=	DBout($row["form_description"]);
		$bc_link_name	=	$row["form_name"];

	} // end if row
	$action = "edit";
} // end if

$meta_title	= "Add New Form";
...
<script language="javascript">
  ...
  <div id="middleContainer">
    <div class="creatAnEventMdl" style="font-size:55px; text-align:center; width:100%"> Add Clinic Form</div>
				...
				<div class="whiteTop">
					<div class="whiteBottom">
						<div class="whiteMiddle" style="padding-top:1px;">
							<div id="accordion">
								<h3><span>ADD FORM INFORMATION</span></h3>
								<div id="box" class="box">

									<div id="head">Form Title</div>
									<div class="ev_title">
										<input type="text" name="product_name" id="product_name" class="" style="width:815px;" value="<?php echo $bc_form_name; ?>"/>
									</div>

											<div id="head">Form Link Name</div>
											<div class="ev_title">
												<input type="text" name="product_code" id="product_code" class="" style="width:815px;" value="<?php echo $bc_link_name; ?>"/>
											</div>

										<div id="head">Form Description</div>
										<div>
