...
if($_POST) {
 ...
 $val2 = '3';
  ...
  $get_dts = mysql_query("update plan set review_status='".$val2."' where id='".$val1."'");

  $chk_final_id = mysql_query("select * from plan where id='".$val1."'");
  while($res_id = mysql_fetch_array($chk_final_id)){
   $pat_id		= $res_id['patient_id'];



  $chk_final_status = mysql_query("select * from plan where clinic_id='".$_SESSION['LOGGEDIN_MEMBER_ID']."' && patient_id='".$pat_id."' && review_status=3");
